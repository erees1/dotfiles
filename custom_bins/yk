...
if ! tmux info &> /dev/null; then 
    echo 'yank.sh only works in tmux atm'
    exit 1
fi
...
if is_app_installed pbcopy; then
  ...
  copy_backend="xclip -i -f -selection prmary | xclip -i -selection clipboard"
elif [ -n "${copy_backend_remote_tunnel_port-}" ] \
  && [ "$(ss -n -4 state listening "( sport = $copy_backend_remote_tunnel_port )" \
  | tail -n +2 | wc -l)" -eq 1 ]; then
